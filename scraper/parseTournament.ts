...
export function parseTournament(
  ...
  const roundOf16Container = $(`h3 > span#${roundOf16Selector}`)
    .parent()
    .nextAll("div")
    .first();
  ...
  const countRo16 = Array.from(playerMap.values()).filter(
    (p) => p.bestFinish === "ro16"
  ).length;
  assert(countRo16 === 8, `Expected 16 players in Ro16, but got ${countRo16}`);

  const countRo8 = Array.from(playerMap.values()).filter(
    (p) => p.bestFinish === "ro8"
  ).length;

  assert(countRo8 === 4, `Expected 8 players in Ro8, but got ${countRo8}`);

  const countRo4 = Array.from(playerMap.values()).filter(
    (p) => p.bestFinish === "ro4"
  ).length;
  assert(countRo4 === 2, `Expected 4 players in Ro4, but got ${countRo4}`);

  const countFinalists = Array.from(playerMap.values()).filter(
  ...
  const countChampions = Array.from(playerMap.values()).filter(
